...
/* test.solo('client does a small write', async function (t) {
  t.plan(8)
  ...
  server.on('request', function (req, res) {
    ...
    res.on('end', () => {
      ...
      t.alike(body, expected, 'server response ended')
    })
  ...
  const postData = b4a.alloc(32, 'abcd')
...
/* test.skip('client does a big write', async function (t) {
  ...
  const firstHalf = b4a.alloc(2 * 1024 * 1024, 'abcd')
  const secondHalf = b4a.alloc(2 * 1024 * 1024, 'efgh')
  ...
  const req = http.request({
    ...
    path: '/',
    headers: {
      'Content-Length': firstHalf.byteLength + secondHalf.byteLength
    }
  }, function (res) {
    t.is(res.statusCode, 200)
